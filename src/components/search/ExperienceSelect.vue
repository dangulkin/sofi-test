		...
		<div class="flex-1">
			<ComboboxRoot v-model="selected" multiple :options="props.options"
				...
				<ComboboxAnchor class="w-full">
					<div
						class="w-full h-12 relative bg-gray-50 border border-gray-200 rounded-xl hover:border-gray-400 transition-all outline-none focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent overflow-hidden"
						tabindex="0">
						<div class="absolute inset-0 flex items-center gap-1 px-1.5 pr-16">
							<template v-if="selected.length">
								<span v-for="level in selected" :key="level"
									...
									{{ props.labels[level] }}
									<button type="button" class="ml-1 focus:outline-none cursor-pointer"
						...
						<ComboboxViewport class="p-1">
							<ComboboxGroup>
								<ComboboxItem v-for="level in props.options" :key="level" :value="level"
									class="relative flex items-center px-3 py-2.5 rounded-lg text-sm outline-none select-none data-highlighted:bg-blue-50 data-highlighted:text-blue-900 data-[state=checked]:font-medium transition-colors cursor-pointer">
